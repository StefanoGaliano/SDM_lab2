...
The entire pipeline is automated by a single shell script (\texttt{run.sh}),
which requires only Docker and Python to be installed.  Running it performs
four steps in sequence:

\begin{enumerate}[noitemsep]
  \item Starts (or reuses) a \texttt{ontotext/graphdb:10.7.6} Docker container
        on port 7200, configured with \texttt{-Xmx2000m} and the large-upload
        size flag.
  \item Installs Python dependencies (\texttt{rdflib}, \texttt{requests}).
  \item Builds the RDF graph and serialises it to \texttt{biomedical\_kg.ttl}.
  \item Programmatically creates the \texttt{biomedical} repository in GraphDB
        (if it does not exist) and uploads the Turtle file via the REST API.
\end{enumerate}

\begin{lstlisting}[style=sparql, caption={run.sh — single entry point.},
                   morekeywords={docker,pip,python3,run,start}]
#!/usr/bin/env bash
docker run -d --name graphdb -p 7200:7200 \
  -e GDB_JAVA_OPTS="-Xmx2000m -Xms1000m \
      -Dgraphdb.workbench.maxUploadSize=40000000000" \
  ontotext/graphdb:10.7.6
...
The knowledge graph is built with the \texttt{rdflib} library
(\texttt{build\_kg.py}) and uploaded to GraphDB via the REST API.
The script has five logical blocks:
...
# Repository creation via Turtle config (multipart POST)
requests.post("http://localhost:7200/rest/repositories",
    files={"config": ("config.ttl", REPO_CONFIG_TTL, "text/turtle")})

# Triple upload
with open("biomedical_kg.ttl", "rb") as f:
    requests.post("http://localhost:7200/repositories/biomedical/statements",
        data=f, headers={"Content-Type": "text/turtle"})
\end{lstlisting}

The repository is created with the \textbf{RDFS (Optimized)} ruleset
...
matches nodes that carry exactly that label.  For the Cypher query to return
all 50 drug nodes, the data loader must have added the \texttt{:Drug} label to
every antibiotic, analgesic, steroid, etc.\ node at creation time.  If a new
drug subclass is added later, the Cypher query automatically includes its nodes
only if the loader remembered to apply the \texttt{:Drug} label — there is no
schema to enforce this.  In SPARQL, adding a new subclass
(\texttt{bio:Antifungal rdfs:subClassOf bio:Drug}) is sufficient; the query
needs no modification.

\subsubsection*{Inferences applied by GraphDB}

The query pattern \texttt{?drug rdf:type bio:Drug} triggers RDFS rule
\textbf{rdfs9}:

\[
  \frac{X\ \texttt{rdfs:subClassOf}\ Y \quad a\ \texttt{rdf:type}\ X}
       {a\ \texttt{rdf:type}\ Y}
\]

None of the 50 drug instances are explicitly typed as \texttt{bio:Drug} in the
uploaded Turtle file; they are typed only as their leaf class.  GraphDB
materialises the following inferred triples (one per instance, five subclass
chains):
...
bio:Oseltamivir      rdf:type  bio:Drug .  # via Antiviral        subClassOf Drug
bio:Prednisone       rdf:type  bio:Drug .  # via Steroid          subClassOf Drug
# ... 45 more (one per remaining instance)
\end{lstlisting}

\textbf{Is this what was expected?}\ Yes.  The RDFS (Optimized) ruleset
guarantees rdfs9 is applied transitively.  With only one level of subclassing
here, each inferred \texttt{rdf:type bio:Drug} triple is produced in a single
rule application.  The query returns exactly 50 results, as expected.
...
bio:Migraine           rdf:type  bio:Disease .  # via NeurologicalDisease subClassOf Disease
# ... 46 more
\end{lstlisting}

\textbf{Is this what was expected?}\ Yes.  The four disease subclasses cover all
50 instances and the inference chain is one hop, so GraphDB produces exactly 50
inferred \texttt{rdf:type bio:Disease} triples and the query returns 50 results.
...
\textbf{Rule rdfs7} (subproperty propagation):
\[
  \frac{P\ \texttt{rdfs:subPropertyOf}\ Q \quad a\ P\ b}
       {a\ Q\ b}
\]

Applied to every explicit relationship triple, this produces 76 inferred
...
\textbf{Is this what was expected?}\ Yes, and it is the most compelling
demonstration of RDFS reasoning in this graph.  The property
\texttt{bio:affects} is a pure ontological abstraction — it exists only in the
schema and is never typed into the data layer.  Yet querying it against a live
GraphDB repository with RDFS (Optimized) returns all 76 drug–disease
interactions, sourced from three semantically distinct relationship types,
without any \texttt{UNION} or enumeration in the query.
